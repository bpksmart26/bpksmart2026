...
| 경로 | 변경 | 책임 |
|---|---|---|
| `apps_script/Code.gs` | 수정 | UNIFIED_COLS 확장, doPost 새 케이스(`sendGuideNow`), updateQt 후크, 마이그레이션 호출 |
...
// 견적 발급 시 updateQt 후크(pdfUrl 있을 때)에서 호출됨
// ─────────────────────────────────────────────────────────────
...
### Task 2.8: updateQt 후크 — PDF Drive 저장 후 트리거

**Files:**
- Modify: `apps_script/Code.gs` updateQt 케이스 (기존 line 157-170 영역)

**왜 saveQt가 아니라 updateQt인가:**
- 견적 PDF는 클라이언트 `genPDF()`에서 생성 → Drive 업로드 → `updateQt(pdfUrl=...)` 호출
- 이 시점이 "PDF가 Drive에 저장된 직후"
- saveQt는 견적 시트 row 저장 시점이라 PDF 아직 없음

- [ ] **Step 1: updateQt 케이스에 후크 추가**

`apps_script/Code.gs`의 updateQt 케이스 (line 157~170 영역)에서 기존 `pushToNotion` `_safeSync` 다음에 추가:
...
**주의:** 기존 코드의 `break;` **위에** 삽입.

- [ ] **Step 2: 검증 — 견적 발급 → PDF 다운로드 → Drive 업로드 → 가이드 자동 생성**
  - 공급기업_관리.html에서 「견적서 발급」 클릭
  - PDF 다운로드 + 토스트 "Drive 백업 완료" 확인
  - Apps Script 로그에서 `[updateQt] generateGuide` 메시지 확인
  - 5-15초 후 통합정보 시트 + 노션에 가이드 컬럼 채워짐 확인

- [ ] **Step 3: 임시저장(saveQDraft)은 가이드 생성 안 함 검증**
  - 모달에서 「임시저장」 클릭 (status='작성중')
  - updateQt 호출되지만 pdfUrl 없음
  - 로그에 generateGuide 호출되지 않음 확인

- [ ] **Step 4: 커밋**

```bash
git add apps_script/Code.gs
git commit -m "fix(guide): generateGuide 후크를 saveQt에서 updateQt(pdfUrl 있을 때)로 이동"
```
...
### Task 3.6: 클라이언트 PDF 진행 모달

**Files:**
- Modify: `공급기업_관리.html` `genPDF()` 함수 + 새 헬퍼 함수
- (선택) Modify: `common.js` 헬퍼 함수 추가

견적서 발급/재발급 시 PDF 생성·다운로드·Drive 업로드 진행 상황을 차단형 모달로 표시.

- [ ] **Step 1: 진행 모달 헬퍼 함수 추가**

`공급기업_관리.html` 의 `<script>` 영역 또는 `common.js`에:

```javascript
// PDF 생성 진행 모달 — 단계별 진행 + UI 차단
function showPdfProgress(stage, totalStages, label) {
  ...
  if (!el) {
    ...
    el.style.cssText = 'position:fixed;inset:0;background:rgba(15,23,42,.55);z-index:99999;display:flex;align-items:center;justify-content:center;backdrop-filter:blur(2px)';
    el.innerHTML = `
      <div style="background:#fff;border-radius:14px;padding:32px 36px;min-width:360px;max-width:480px;box-shadow:0 24px 64px rgba(0,0,0,.25)">
        <div style="font-size:13px;color:#64748b;letter-spacing:1px;font-weight:700;margin-bottom:6px">BPK SMART 2026</div>
        ...
        <div id="pdf-progress-label" style="font-size:13.5px;color:#475569;line-height:1.5"></div>
...
function hidePdfProgress(success, message) {
  ...
  if (success) {
    ...
    document.getElementById('pdf-progress-stage').innerHTML = '<button onclick="document.getElementById(\'pdf-progress-modal\').style.display=\'none\'" style="margin-top:14px;padding:8px 18px;background:#dc2626;color:#fff;border:none;border-radius:6px;cursor:pointer;font-weight:600">닫기</button>';
  } else {
    // 즉시 닫기
...
- [ ] **Step 2: genPDF 흐름에서 사용**

`genPDF` 함수의 기존 `showApiLoading` 호출들을 `showPdfProgress`로 교체:
  - 단계 1/5: `showPdfProgress(1, 5, '폰트 준비 중...')`
  - 단계 2/5: `showPdfProgress(2, 5, '사진 로딩 중... (' + count + '장)')`
  - 단계 3/5: `showPdfProgress(3, 5, '견적서 PDF 생성 중...')`
  - PDF 다운로드 후 단계 4/5: `showPdfProgress(4, 5, '파일 다운로드 완료')`
  - Drive 업로드 시작 단계 5/5: `showPdfProgress(5, 5, 'Drive 백업 중...')`
  - 성공 시: `hidePdfProgress(true, '✓ Drive에 저장 완료')`
  - 실패 시 (catch): `hidePdfProgress(false, e.message)`

기존 `hideApiLoading()` 호출은 `hidePdfProgress(true, ...)`나 `hidePdfProgress(false, ...)`로 교체.

- [ ] **Step 3: 검증**
  - 견적서 발급 클릭 → 진행 모달 표시 확인
  - 단계별 progress bar 채워지는 것 확인
  - 모달 떠있는 동안 다른 버튼 클릭 무반응 확인 (UI 차단)
  - Drive 저장 완료 후 1.5초 뒤 자동 닫기 확인

- [ ] **Step 4: 커밋**

```bash
git add 공급기업_관리.html common.js
git commit -m "feat(ui): 견적서 발급 진행 모달 — 단계별 progress + UI 차단"
```
...
- ✅ generateGuide 자동 트리거 — updateQt 후크 with pdfUrl (Task 2.7 + 2.8)
- ✅ pollAndSend (Task 3.4)
- ✅ Make 백업 (Task 3.5 + 5.2)
- ✅ 클라이언트 PDF 진행 모달 (Task 3.6)
